...
async def process_meeting_scheduling(websocket: WebSocket, entities: dict, session: dict):
    """Enhanced meeting scheduling with better feedback"""
    try:
...
async def fill_missing_fields_async(entities: dict, text_to_voice, websocket, session: dict) -> dict:
    """Async wrapper for fill_missing_fields with session context"""
    try:
